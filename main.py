...
@dp.callback_query_handler(lambda query: query.data == 'button_karta')
async def button_pressed_handler(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await ButtonState.Karta_button.set()


@dp.message_handler(Text, state=ButtonState.Karta_button)
async def text_handler(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(message.chat.id,"Введено не допустимое значение")
...
async def functions_bot(message: types.Message):
    if 'Касса' in message.text:
        ...
        await bot.send_message(message.chat.id, f"Баланс кассы сейчас: {cassa[0]} руб",
                               reply_markup=marcups.inline_Cassa)
    ...
    elif 'Баланс карты' in message.text:
        count = db.select_karta()
        await  bot.send_message(message.chat.id,f'Баланс карты: {count}',reply_markup=marcups.inline_Karta)
...
async def process_callback_kb1btn1(callback_query: types.CallbackQuery):
    ...
    if code == 2:
        ...
        await bot.answer_callback_query(callback_query.id)
    if code == 3:
        document = types.InputFile('Karta_history.txt')
        await bot.send_document(chat_id=callback_query.from_user.id, document=document)
        await bot.answer_callback_query(callback_query.id)
